...
<div class="card card-outline card-primary">
    <div class="card-body table-responsive no-padding">
        <div class="col-md-12 col-xl-12 col-lg-12 col-xs-12 col-sm-12 text-right">
            <a href="{{ route('dashboard.news.add') }}" class="btn btn-primary rounded-0"><i class="fas fa-plus-circle"></i> Adicionar</a>
        </div>
        <div class="col-md-12 col-xl-12 col-lg-12 col-xs-12 col-sm-12 mt-3">
            <table class="table table-striped">
                    ...
                    @if(count($data) > 0)
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('dashboard.news.edit',$item->id) }}" class="btn btn-info"><i class="far fa-edit"></i></a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" id="button_del_{{ $item->id }}" onclick="deleteRegister({{ $item->id }},'{{ route('dashboard.news.del.send', $item->id) }}')" class="btn btn-danger @if($item->status == 0) disabled @endif"><i class="fas fa-trash-alt"></i></a>
                                </td>
                            </tr>
                        </tbody>
                        @endforeach
                    ...
                    <tbody>
                        <tr>
                            <td colspan="5" align="center">Não possui registro.</td>
                        </tr>
                    </tbody>
                    @endif
            </table>
