    ...
    <div class="card-body table-responsive no-padding">
        <form action="{{ route($route) }}" method="post" enctype="multipart/form-data" id="form">
            ...
            <div class="form-group">
                ...
                <input type="password"  name="password" id="password" class="@error('password') is-invalid @enderror form-control border border-top-0 border-left-0 border-right-0 rounded-0" placeholder="Senha" @if(!empty($data->password)) value="{{ $data->password }}"  @endif>
                @error('password')
                    <span class="error invalid-feedback">
                    <strong>{{ $message }}</strong>
                    </span>
            ...
            <div class="form-group">
                ...
                <select name="role" class="@error('role') is-invalid @enderror form-control border border-top-0 border-left-0 border-right-0 rounded-0" id="role">
                    <option value="">Selecione</option>
                    <option value="1" @if(!empty($data->role)) @if($data->role == 1) selected @endif @endif>Administrador</option>
                    <option value="2" @if(!empty($data->role)) @if($data->role == 2) selected @endif @endif>Comum</option>
                </select>
                @error('role')
                    <span class="error invalid-feedback">
                    <strong>{{ $message }}</strong>
