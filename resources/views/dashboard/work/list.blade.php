...
<div class="card card-outline card-primary">
    <div class="card-body table-responsive no-padding">
        <div class="col-md-12 col-xl-12 col-lg-12 col-xs-12 col-sm-12 text-right">
            <a href="{{ route('dashboard.curriculum.add') }}" class="btn btn-primary rounded-0"><i class="fas fa-plus-circle"></i> Adicionar</a>
        </div>
            ...
            <table class="table table-striped">
                    <thead class="thead-dark">
                        <tr>
                            ...
                            <th scope="col"  width="5%" colspan="3" align="center"><center>Ações</center></th>
                        </tr>
                    </thead>
                    @if(count($data) > 0)
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('dashboard.curriculum.edit',$item->id) }}" class="btn btn-info"><i class="far fa-edit"></i></a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" id="button_del_{{ $item->id }}" onclick="deleteRegister({{ $item->id }},'{{ route('dashboard.curriculum.del.send', $item->id) }}')" class="btn btn-danger @if($item->status == 0) disabled @endif"><i class="fas fa-trash-alt"></i></a>
                                </td>
                                <td>
                                    <a href="{{ url('storage/'.$item->path)}}" target="_blank" class="btn bg-purple"><i class="fas fa-download"></i></a>
                                </td>
                            </tr>
                        </tbody>
                        @endforeach
